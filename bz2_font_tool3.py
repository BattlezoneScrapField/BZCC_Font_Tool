...
def render_font_file(face, pixel_size, config):
    ...
    # +10 base compensation for BZ fonts vs TTF metrics
...
class FontToolApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("BZCC Font Tool Pro")
        self.setMinimumSize(700, 300)
        self.resize(800, 400)
        self.setAcceptDrops(True)
        self.font_files = {}
        self.worker = None
        self.app_settings = QSettings("VAC_Dominator", "BZCC_Font_Tool_v3")
        ...
        self.default_profiles = {
            'Tiny':   {'size_offset': 0, 'stroke': 0, 'letter_spacing': 0, 'contrast': 2.0, 'brightness': 1.0, 'use_aa': False, 'sharpen': True},
            'System': {'size_offset': 0, 'stroke': 0, 'letter_spacing': 0, 'contrast': 2.0, 'brightness': 1.0, 'use_aa': False, 'sharpen': True},
            'Small':  {'size_offset': 0, 'stroke': 0, 'letter_spacing': 0, 'contrast': 2.0, 'brightness': 1.0, 'use_aa': False, 'sharpen': True},
            'Medium': {'size_offset': 0, 'stroke': 0, 'letter_spacing': 0, 'contrast': 2.0, 'brightness': 1.0, 'use_aa': False, 'sharpen': True},
            'Large':  {'size_offset': 0, 'stroke': 0, 'letter_spacing': 0, 'contrast': 2.0, 'brightness': 1.0, 'use_aa': False, 'sharpen': True}
        }
    ...
    def dropEvent(self, event):
        ...
        elif os.path.isfile(path) and path.lower().endswith(('.ttf', '.otf', '.ttc', '.fon', '.fnt', '.pfb', '.pfa', '.dfont')):
    ...
    def init_dock_window(self):
        pass

    def init_ui(self):
        # 1. Central Widget is now the Live Preview (consumes all extra resize space beautifully)
        ...
        # 2. Controls go into a Top Dock Widget (always stays compact, no black voids)
        self.controls_dock = QDockWidget("Control Panel [Detach/Dock via Title Bar]", self)
        ...
        self.controls_dock.setObjectName("ControlsDockWindow")
        ...
        config_group = QGroupBox("Font & Profile Settings")
        ...
        self.btn_load_font = QPushButton("Load TTF")
        ...
        self.slider_offset, self.lbl_val_offset = self.create_grid_slider(settings_grid, 2, 0, "Size Offset", -10, 10, 0)
    ...
    def closeEvent(self, event):
        ...
        self.app_settings.setValue("dock_state", self.saveState())
    ...
    def refresh_fonts(self):
        ...
        # 1. Search locally and recursively in tool folder
        ...
        for root, _, files in os.walk(tool_dir):
            for f in files:
                if f.lower().endswith(valid_exts):
                    ...
                    try:
                        face = freetype.Face(path)
                        family = face.family_name.decode(errors='replace') if face.family_name else "Unknown"
                        style = face.style_name.decode(errors='replace') if face.style_name else ""
                        display_name = f"{family} {style}".strip() if style and style not in family else family
        ...
        # Sort fonts, ensuring BZ1 Classics appear at the top
        ...
        other_items = sorted([item for item in items if "★ [BZ1 Classic]" not in item])
        
        self.combo_fonts.clear()
        self.combo_fonts.addItems(bz_items + other_items)
    ...
    def load_custom_font_from_path(self, file_path):
        if file_path and os.path.exists(file_path):
            try:
                ...
                family = face.family_name.decode(errors='replace') if face.family_name else "CustomFont"
                style = face.style_name.decode(errors='replace') if face.style_name else ""
                base_name = f"{family} {style}".strip() if style and style not in family else family
    ...
    def browse_custom_font(self):
        file, _ = QFileDialog.getOpenFileName(self, "Load Custom Font", "", "Fonts (*.ttf *.otf *.ttc *.fon *.fnt *.pfb *.pfa *.dfont)")
        if file:
